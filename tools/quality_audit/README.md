...
이 검수기는 일상적인 문서·국소 기능·단일 컴포넌트 변경의 기본 검증 도구가 아닙니다. 단계 완료, 공개 준비, 공통 아키텍처·권한·전역 내비게이션 변경 또는 사용자가 전체 검수를 요청한 경우에만 실행합니다. 일반 개발 작업은 [메인 개발자 역할 정의](../../docs/MAIN_DEVELOPER_ROLE.md)의 변경 유형별 기본 검증을 따릅니다.

## 자원 사용 방식

- 기본 `--cpu-budget 5.25` (이전 `1.75`의 3배)
- 기본값 `5.25`는 평균 CPU 사용 목표이며, Linux에서는 브라우저 대기를 숨길 수 있도록 최대 12개 논리 CPU affinity를 허용합니다.
- 정적 검사와 독립적인 테스트·빌드는 최대 6개 실행 슬롯에서 병렬 처리합니다.
- 비용이 큰 Node 테스트, Vitest와 Vite 빌드를 우선 배치해 여러 CPU가 함께 일하는 구간을 늘립니다.
- axe 7개 화면과 검색·초점 경로를 독립 작업으로 나눠 긴 단일 브라우저 꼬리를 제거합니다.
- 기본 설정에서는 명령 검사를 먼저 끝낸 뒤 대기형 브라우저 작업을 별도 Chromium 프로필 최대 24개에 가중치 균형 배치합니다. 실제 슬롯 수는 브라우저 작업 수를 넘지 않습니다.
- 로컬 성능 측정은 병렬 Chromium의 간섭을 피하기 위해 마지막에 단독 실행합니다.
- OpenMP, BLAS, Rayon과 Node의 libuv 스레드 수도 최대 6으로 제한합니다.
- 기본 설정은 CPU 작업이 겹치는 동안 이전 목표의 3배인 5.25 Core 활용을 목표로 합니다. 다만 파일·네트워크·브라우저 대기 구간까지 포함한 전체 실행 평균을 보장하는 CPU 예약 방식은 아닙니다.
- affinity는 CPU quota가 아니므로 순간 사용량은 목표값을 넘을 수 있지만, 명령·라이브러리 스레드는 최대 6개로 제한합니다.
- 실행 장비가 제공하는 CPU가 12개보다 적으면 affinity와 실제 사용량은 가용 CPU 수로 자동 제한됩니다.
