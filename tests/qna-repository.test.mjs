...
test("답변 메일은 해당 질문 작성자와 개별 관리자·마스터를 조회하고 부서 규칙은 별도로 표시한다", async () => {
  const calls = []
  const repository = createQnaRepository({ pool: { async execute(sql, params) {
    calls.push([sql, params])
    if (sql.includes('FROM quality_hub_qna_question')) return [[{ questionId: 7, title: '질문', authorUserId: 'general.author' }]]
  ...
  assert.deepEqual(result.recipientUserIds, ['admin', 'master', 'general.author'])
  ...
  assert.match(calls[0][0], /author_user_id AS authorUserId/)
  const questionMail = await repository.getMailContext(7)
  assert.deepEqual(questionMail.recipientUserIds, ['admin', 'master'])
})
