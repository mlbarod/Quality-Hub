...
test("스카이 톤 홈에서 정위치로 분산 배치한 네 개의 App과 중앙 품질 Agent를 제공한다", () => {
  ...
  assert.match(html, /id="quality-agent-floating-panel"[^>]*품질 Agent 작업 공간/)
  assert.match(styles, /\.prototype\[data-layout="top"\]\[data-agent-mode="drawer"\] \.workspace \{\s*margin-right: 0;/)
  assert.match(styles, /\.agent-workspace \{[\s\S]*inset: var\(--header-height\) auto 0 50%;[\s\S]*width: min\(calc\(var\(--agent-drawer-width\) \* 3\), calc\(100vw - 96px\)\);[\s\S]*border-radius: 25px;/)
  assert.match(styles, /\.prototype\[data-agent-mode="drawer"\] \.agent-workspace,[\s\S]*\.prototype\[data-agent-mode="full"\] \.agent-workspace/)
  assert.match(styles, /\.prototype\[data-agent-mode="drawer"\] \.agent-backdrop \{[\s\S]*pointer-events: auto;/)
