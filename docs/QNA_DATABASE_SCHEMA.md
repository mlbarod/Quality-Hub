...
- 등록 직후 응답은 해당 질문·답변의 DB 저장 시각 한 필드만 추가 조회해 반환한다. 전체 목록 재조회 없이 등록 직후와 재진입 시각을 일치시킨다. 아래 한국 시간 기준을 적용한다.
...
- 질문·추가답변·변경 이력·알림의 시각은 `UTC_TIMESTAMP(3) + INTERVAL 9 HOUR`로 산출해 기존 `DATETIME(3)` 열에 한국 시각으로 저장한다. DB 세션·컨테이너의 기본 시간대에 의존하지 않는다.
- API는 DB의 시간대 없는 날짜 문자열에 `+09:00`을 명시한다. 이미 시간대가 있는 값과 `Date` 객체는 해당 시점을 유지한다.
- 게시판 목록·질문 상세·추가답변·삭제 목록·변경 이력은 `Asia/Seoul`로 표시한다. 사용자 기기의 시간대가 달라도 한국 시각을 유지한다.
- 과거 데이터의 실제 저장 시간대는 운영 DB 확인 전에는 알 수 없다. 과거 행을 일괄 이동하지 않으며, UTC 등 다른 기준으로 저장된 행의 보정은 영향 범위를 확인한 뒤 별도 승인으로 진행한다.
- 기존 테이블 기본값은 변경하지 않는다. 포털 외부에서 직접 삽입할 때는 같은 KST 저장 규칙이 필요하다. 서버 시계 자체의 오차(NTP)와 운영 DB 검증은 별도 운영 확인 대상이다.
- 참고: [MySQL 시간대 동작](https://dev.mysql.com/doc/refman/8.4/en/time-zone-support.html).
