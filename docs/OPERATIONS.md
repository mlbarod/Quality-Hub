...
이번 전환은 모든 현재 기능을 제공하는 `시범 운영`이다. Rule&SOP, Q&A와 내부 알림은 각 브라우저의 로컬 저장소에만 유지된다. 실제 SSO 권한 집행과 나머지 사내 데이터 연동, 약 50명 성능·보안·접근성 전체 검증은 아직 완료되지 않았으므로 `공개 준비 완료`로 판정하지 않는다.
...
`/healthz`의 정상 응답은 `{"status":"ok"}`다. 이 경로는 앱 프로세스의 생존 상태만 확인한다. `/readyz`는 DB·RAG·GPT-OSS 필수 환경변수가 모두 입력됐을 때 HTTP 200을 반환하지만 실제 네트워크 연결 성공까지 보장하지 않는다. 최종 연동은 Agent 질문과 Report 목록 조회로 별도 확인한다. 로그에는 접속정보나 SQL 원문을 남기지 않는다.
...
- 현재 접근 통제는 사내망·허용 IP에 의존한다. 화면의 역할 미리보기와 `x-quality-hub-user-id`는 실제 인증 수단이 아니다.
- 허용된 사용자는 역할 값을 바꿀 수 있으므로 관리자 기능을 실제 권한으로 간주하면 안 된다.
- Rule&SOP, Q&A, 알림과 권한 규칙은 사용자·브라우저 간 공유되지 않는다.
- Spotfire iframe 허용 정책과 실제 SSO, 나머지 사내 데이터, 운영 부하·복구 훈련은 후속 단계에서 검증한다.
