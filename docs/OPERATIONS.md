...
파일·import·의존성·빌드 설정 또는 Docker 복사 규칙을 변경한 경우 `npm run build:container-check`를 실행한다. `npm test`에도 동일 검사가 포함된다. 두 Dockerfile의 build 단계에서 복사하는 파일과 현재 `.dockerignore` 규칙을 적용한 임시 폴더에서 각각 실제 Vite 빌드를 실행한다. 이번 `qnaFontSize.mjs` 누락과 제외 규칙에 의한 파일 누락도 실패 사례로 검사한다.

이 검사는 기존 로컬 의존성을 사용하므로 새로 내려받은 의존성, 베이스 이미지와 실제 C-DEP 빌드 성공을 보장하지 않는다. 현재 Dockerfile의 단순 COPY·제외 패턴만 지원하고 미지원 문법은 실패로 알린다. 해당 설정을 바꿀 때는 검사기 갱신 또는 실제 Docker 빌드가 필요하다. 원본 환경파일과 저장소는 수정하지 않으며 임시 폴더는 검사 후 삭제한다.
